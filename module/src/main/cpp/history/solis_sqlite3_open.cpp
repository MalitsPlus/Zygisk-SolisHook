...
typedef void* (*Hook)(cSharpString* dbFilePath, cSharpByteArray* key, int openFlag, void* methodInfo);
Hook backup = nullptr;
// FIXME: ReturnType Args
void* hook(cSharpString* dbFilePath, cSharpByteArray* key, int openFlag, void* methodInfo){
    if(backup == nullptr){
        LOGE("backup DOES NOT EXIST");
    }

    LOGI("====== MAGIC SHOW BEGINS ======");
    LOGI("dbFilePath is %s", getCsharpString(dbFilePath));
    LOGI("key is %s", getByteString(key->buf, key->length));
    LOGI("openFlag is %d", openFlag);

    char txt[4096];
    memset(txt, 0x00, 4096);
    sprintf(txt, "====== MAGIC SHOW BEGINS ======\ndbFilePath is %s\nkey is %s\nopenFlag is %d",
            getCsharpString(dbFilePath),
            getByteString(key->buf, key->length),
            openFlag
    );

    write2File("dbKeys.log", txt, "a+");
    // 原始调用
    void* r = backup(dbFilePath, key, openFlag, methodInfo);
    return r;
}
...
//    Il2CppClass* clazz = clas;
    ...
    Il2CppClass* clazz = il2cpp_class_from_name(image, "Qua.Master.SQLite", "SQLite3Manager");
    ...
    MethodInfo* methodInfo = (MethodInfo*)il2cpp_class_get_method_from_name(final_clazz, "Open", 3);
