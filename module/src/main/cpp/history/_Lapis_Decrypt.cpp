...
int isGame(JNIEnv *env, jstring appDataDir) {
    ...
    if (sscanf(app_data_dir, "/data/%*[^/]/%d/%s", &user, package_name) != 2) {
        if (sscanf(app_data_dir, "/data/%*[^/]/%s", package_name) != 1) {
            package_name[0] = '\0';
            LOGW("Riru-hook: can't parse %s", app_data_dir);
            return 0;
        }
    }
    env->ReleaseStringUTFChars(appDataDir, app_data_dir);
    if (strcmp(package_name, game_name) == 0) {
        LOGD("Riru-hook: detect game: %s", package_name);
        return 1;
    }
    else {
        return 0;
...
// FIXME: 修改为与hook的函数一样的参数
typedef void (*Hook)(cSharpByteArray* array, int key, int start, int size, int startIdx, void* methodInfo);
Hook backup = nullptr;
// FIXME: 修改为与hook的函数一样的参数
void hook(cSharpByteArray* array, int key, int start, int size, int startIdx, void* methodInfo){
    if(backup == nullptr){
    ...
    LOGE("Riru-hook: array at %lx, key=(%d), start=(%d), size=(%d), startIdx=(%d)", (long)array, key, start, size, startIdx);
    // 原始调用
    backup(array, key, start, size, startIdx, methodInfo);
...
void hook_each(unsigned long rel_addr, void* hook, void** backup_){
    LOGI("Riru-hook: hook_each: Installing hook at %lx", rel_addr);
    ...
    if (-1 == mprotect(page_start, PAGE_SIZE, PROT_READ | PROT_WRITE | PROT_EXEC)) {
        LOGE("Riru-hook: set mprotect failed(%d)", errno);
        return ;
    }
    LOGI("Riru-hook: mprotect set successfully");
...
{
    // tid
    LOGI("Riru-hook: hack thread: %d", gettid());
    srand(time(nullptr));
    LOGI("Riru-hook: start getting loader_dlopen");
    // 这个symbol_name 是哪来的？？
    void* loader_dlopen = DobbySymbolResolver(nullptr, "__dl__Z9do_dlopenPKciPK17android_dlextinfoPKv");
    LOGI("Riru-hook: loader_dlopen got");

    LOGI("Riru-hook: start hook_each 'loader_dlopen'");
    hook_each((unsigned long)loader_dlopen, (void*)dlopen_, (void**)&dlopen_backup);
    LOGI("Riru-hook: hook_each 'loader_dlopen' completed");

    LOGI("Riru-hook: start find addr of module 'libil2cpp.so'");
    while (true)
    {
    ...
    LOGD("Riru-hook: detect libil2cpp.so %lx, start sleep", base_addr);
    ...
    sleep(2);
    LOGW("Riru-hook: hack game begin");
    ...
    // FIXME: 修改为需要hook的类
//    Il2CppClass* clazz = il2cpp_class_from_name(image, "Oz.GameFramework.Runtime.Resource", "LapisExtern");
    Il2CppClass* clazz = clas;
    LOGI("Riru-hook: clazz got at %lx", (long)clazz);
    // 获取 Class 中的指定 MethodInfo，取其中的 methodPointer 获取地址，并 hook
    // 关于 MethodInfo 的结构，可参照 il2cpp 源码 il2cpp-class-internals.h#341
    // FIXME: 修改为需要hook的方法，args个数也需要修改（-1为忽略
    MethodInfo* methodInfo = (MethodInfo*)il2cpp_class_get_method_from_name(clazz, "_Lapis_Decrypt", -1);
    LOGI("Riru-hook: methodInfo got at %lx", (long)methodInfo);

//    void* *it = (void* *)malloc(1);
//    MethodInfo* m = (MethodInfo*)il2cpp_class_get_methods(clazz, it);
//    LOGI("Riru-hook: methodInfo got at %lx, name %s", (long)m, m->name);

    unsigned long addr = (unsigned long)methodInfo->methodPointer;
    LOGI("Riru-hook: method got at %lx", addr);

    LOGI("Riru-hook: start hook target method...");
    hook_each(addr, (void*)hook, (void**)&backup);

    LOGD("Riru-hook: hack game finish");
    return nullptr;
}