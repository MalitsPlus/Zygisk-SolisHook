...
typedef void (*Hook)(void* self, cSharpString* rootcert, void* keyCertPair, void* callBack, void* methodInfo);
Hook backup = nullptr;
// FIXME: ReturnType Args
void hook(void* self, cSharpString* rootcert, void* keyCertPair, void* callBack, void* methodInfo){
    if(backup == nullptr){
        LOGE("backup DOES NOT EXIST");
    }
    LOGI("====== MAGIC SHOW BEGINS ======");
    // 原始调用
    backup(self, rootcert, keyCertPair, callBack, methodInfo);
    LOGI("self at %lx", (long)self);
    LOGI("cert: %s", rootcert->buf);
}
...
void *hack_thread(void *arg)
{
    ...
    // FIXME: Assembly Name
    while(strcmp((*assembly_list)->aname.name, "Grpc.Core.Api") != 0){
...
//    Il2CppClass* clazz = clas;
    ...
    Il2CppClass* clazz = il2cpp_class_from_name(image, "Grpc.Core", "SslCredentials");
    ...
    MethodInfo* methodInfo = (MethodInfo*)il2cpp_class_get_method_from_name(final_clazz, ".ctor", 3);
